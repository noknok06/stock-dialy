...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initSwipeDetection() {
      ...
      document.body.addEventListener('htmx:afterSwap', function(evt) {
        const newElements = evt.detail.target.querySelectorAll('.diary-card');
    ...
    function setupSwipeHandlers(element) {
      // すでに設定済みならスキップ
      if (element.dataset.swipeInitialized === 'true') return;
    ...
    function initTouchFeedback() {
      ...
      document.querySelectorAll(clickableSelector).forEach(elem => {
      ...
      // 新しく追加される要素にも適用
      document.body.addEventListener('htmx:afterSwap', function(evt) {
        evt.detail.target.querySelectorAll(clickableSelector).forEach(elem => {
          if (!elem.classList.contains('touch-enhanced')) {
            elem.classList.add('touch-enhanced');
