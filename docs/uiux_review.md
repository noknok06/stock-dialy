...
### 3.5 レイアウト特化の見やすさ改善（実施済み・2026-06）

コード監査（home/detail/デザイントークンの3系統）で判明した阻害要因への対処。6コミットに分割して実施。

1. **極小フォントの正規化**: 0.62-0.72rem（10-11.5px相当）の本文系テキスト約30箇所を
   typography.css トークン水準の 0.75rem へ引き上げ。タブアイコン内の位置調整済みバッジのみ除外。
2. **ページ本文幅の2段階統一**: 従来 timeline 760px / dashboard 1280px / detail 1600px / home 無制限と
   ページ遷移のたび本文幅が激変していたのを「読み物=860px（timeline, diary_summary）/
   作業=1320px（home, detail, dashboard）」に統一。`common-base.css` に `.page-narrow` / `.page-wide`
   ユーティリティを新設。detail はインラインCSSと diary-detail.css の二重定義を両方変更。
   home はサイドバー折りたたみ時にカードが約770px幅へ間延びする現象も解消。
3. **冗長表現の削減**: 取引履歴の信用取引注釈（バッジと完全重複、件数分繰り返し）を削除。
   関連グラフのフィルタチェック5個を折り返しから横スクロールチップ行へ。
4. **detail モバイル圧縮**: ヘッダー+タブの占有を約230-270px→約170-190pxに。
   タブを均等幅化（min-width 120px→0 + flex:1）し、375px幅で6タブが横スクロールなしに収まる。
   実現損益カードはラベル左・値右の1行レイアウトに。
5. **日記カードの情報階層整理**: 損益ピルをタイトル行右端へ移動しメタ行のwrap乱れを解消。
   開示インジケータをアイコン+日付に簡略化（詳細は title 属性）。タグ行とフッターの区切り線二重を解消。
6. **home モバイル検索エリア圧縮**: 約140-160px→約90-100px。ハッシュタグチップをデフォルト
   折りたたみ化し「@タグ」トグルで展開（DOMに残すためフィルタ同期JS・HTMXに影響なし）。
   ファーストビューに最初の日記カードが入るようになった。

スコープ外と判断したもの: ダークモード二重定義の解消・インラインスタイルの全面外部化
（見た目が変わらない保守リファクタのため）、diary_form のステップ化（機能再設計を伴うため別論点）。
