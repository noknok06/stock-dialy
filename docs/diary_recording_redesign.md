...
> **実装ギャップ（コードはこの再定義に未追従・要対応）**：`StockDiary.reason` の `verbose_name` は今も `'投資理由'`
> （`models.py`）／`diary_form` のガイドモード（投資仮説・カタリスト・反論・損切り＝方向性B）は reason を
> *購入理由の器*として構造化しており、背景への再定義と矛盾する。実装段階で、reason のラベル/ヘルプを背景寄りへ、
> ガイドモードの購入理由項目は `topic` スレッド（または Thesis）側へ移す。
