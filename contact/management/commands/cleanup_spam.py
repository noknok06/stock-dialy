...
# contact/management/commands/cleanup_verification.py
from django.core.management.base import BaseCommand
from django.utils import timezone
from contact.models import ContactMessage
import datetime
...
class Command(BaseCommand):
    help = '期限切れの未認証メッセージを削除します'

    def add_arguments(self, parser):
        parser.add_argument(
            '--hours',
            type=int,
            default=48,  # デフォルト48時間
            help='何時間前の期限切れメッセージを削除するか（デフォルト: 48時間）'
        )
        parser.add_argument(
            '--dry-run',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        expired_unverified = ContactMessage.objects.filter(
            is_verified=False,
            verification_expires_at__lt=cutoff_time
        )
        ...
        if count == 0:
            self.stdout.write(
                self.style.SUCCESS(f'{hours}時間前より古い期限切れ未認証メッセージはありません。')
            )
            return
        
        if dry_run:
            self.stdout.write(
                self.style.WARNING(f'削除対象: {count}件の期限切れ未認証メッセージ')
            )
            for msg in expired_unverified[:10]:  # 最初の10件を表示
                expired_hours = (timezone.now() - msg.verification_expires_at).total_seconds() / 3600
                self.stdout.write(
                    f'  - {msg.created_at}: {msg.name} ({msg.email}) '
                    f'[期限切れ: {expired_hours:.1f}時間前]'
                )
            if count > 10:
                self.stdout.write(f'  ... その他 {count - 10}件')
        else:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'{count}件の期限切れ未認証メッセージを削除しました。')
            )
            
            # ログ出力
            import logging
            logger = logging.getLogger(__name__)
            logger.info(f'期限切れ未認証メッセージ削除: {count}件削除（{hours}時間以上前に期限切れ）')